...
_get_connection_status() {
    MULLVAD_VPN_STATUS=$(mullvad status | head -n1 | sed -e 's/\s.*$//' | tr '[:upper:]' '[:lower:]')
    if echo "connecting connected disconnected blocked" | grep -wi "$MULLVAD_VPN_STATUS" > /dev/null; then
...
get_connection_status() {
    echo "󰖂   $(_get_connection_status | tr '[:lower:]' '[:upper:]')"
}
...
case $1 in
	...
	"--toggle")
	    if [ "$(_get_connection_status)" = "connected" ]; then
	        mullvad disconnect
	elif [ "$(_get_connection_status)" = "disconnected" ]; then
	        mullvad connect
	    else
            :
	    fi;;
